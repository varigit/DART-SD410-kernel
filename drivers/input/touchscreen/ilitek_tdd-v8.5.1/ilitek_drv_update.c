...
static u8 _gOneDimenFwData[MSG28XX_FIRMWARE_WHOLE_SIZE*1024] = {0}; // for MSG28xx : array size = MSG28XX_FIRMWARE_WHOLE_SIZE*1024

static u8 _gFwDataBuf[MSG28XX_FIRMWARE_WHOLE_SIZE*1024] = {0}; // for update firmware(MSG28xx/MSG58xxa) from SD card
...
void DrvOptimizeCurrentConsumption(void)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2118A) // CHIP_TYPE_MSG58XXA not need to execute the following code
    {
        mutex_lock(&g_Mutex);
...
u16 DrvGetChipType(void)
{
    ...
    // Check chip type by using DbBus for MSG28XX/MSG58XX/MSG58XXA/ILI2117A/ILI2118A.
...
s32 DrvUpdateFirmwareCash(u8 szFwData[][1024], EmemType_e eEmemType)
{
    ...
    if (g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2118A) // (0x85) || (0xBF) || (0x2117) || (0x2118)
    {
...
s32 DrvUpdateFirmwareBySdCard(const char *pFilePath)
{
    ...
    {
        DBG(&g_I2cClient->dev, "Please check hex file\n");
        filp_close(pfile, NULL);
        return -1;
    }
    // read firmware
    ...
    {
        if ((g_ChipType == CHIP_TYPE_MSG28XX || g_ChipType == CHIP_TYPE_MSG58XXA || g_ChipType == CHIP_TYPE_ILI2117A || g_ChipType == CHIP_TYPE_ILI2118A) && fsize == 133120/* 130KB */)
        {
...
static void _DrvUpdateFirmwareBySwIdDoWork(struct work_struct *pWork)
{
    ...
    else // nRetVal == -1 for MSG28xx/MSG58xxa or nRetVal == 2/3/4 for ILI21xx
    {
        _gUpdateRetryCount --;
