...
static int new_flow_start_test(void)
{
    ...
    if(ERR_ALLOC_MEM(short_deltac_data)) {
        ...
        if(run_type == RUN_SHORT_TEST) {
            ...
            DebugShowArray2(deltac_data, MAX_SUBFRAMES_30XX * MAX_AFENUMs_30XX, -32, 10, 10, "hort Dump2 - Dump1");
        }
    if(ret == 0)
    {
        /* Judge values */
        ...
        else if(run_type == RUN_OPEN_TEST)
        {
            ...
            mp_info("%d,%d\n", mp_test_result->pdeltac_buffer[0], deltac_data[0]);
...
int uniformity_test(void) {
    ...
    mp_test_data->Unifor_BG_Golden_Min = (mp_test_data->Unifor_BG_Golden * (100 - mp_test_data->Unifor_BG_Sum_Range))/100;
    mp_test_data->Unifor_BG_Golden_Max = (mp_test_data->Unifor_BG_Golden * (100 + mp_test_data->Unifor_BG_Sum_Range))/100;
    mdelay(50);
    StartMCU();
    mdelay(10);
    RegSet16BitValue(0x1402, GET_BG_SUM);
    StopMCU();
    RegSet16BitValue(0X3D08, 0xFEFF);   // 
    mp_test_result->Unifor_sum = RegGet16BitValue(0x13C4);
    if(mp_test_result->Unifor_sum < mp_test_data->Unifor_BG_Golden_Min || 
       mp_test_result->Unifor_sum > mp_test_data->Unifor_BG_Golden_Max ) {
        nRetVal = ITO_TEST_FAIL;
    }
    mp_info("Uniformity BG sum = %d\n", mp_test_result->Unifor_sum);
    mp_info("mp_test_data->Unifor_BG_Golden_Min = %d\n", mp_test_data->Unifor_BG_Golden_Min);
    mp_info("mp_test_data->Unifor_BG_Golden_Max = %d\n", mp_test_data->Unifor_BG_Golden_Max);   
    return nRetVal;       
}
